...
export const MissionVariablesTab: React.FC<MissionVariablesTabProps> = ({
    ...
    return (
        <section className="space-y-4 border border-border/50 p-6 rounded-xl bg-card shadow-sm">
            <h2 className="text-title text-white border-b border-border/40 pb-2 mb-4 block flex items-center gap-1.5">
            ...
            <div>
                <textarea
                    className={`w-full h-64 font-mono rounded-md border bg-[#13161B] px-3 py-2 text-body focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${jsonError ? 'border-destructive' : 'border-input'}`}
                    value={variablesJson}
                    onChange={(e) => handleVarChange(e.target.value)}
                />
                {jsonError && (
                    <span className="text-destructive text-body mt-1 block">
                        {jsonError}
                    </span>
                )}
            </div>
