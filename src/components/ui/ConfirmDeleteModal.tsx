...
interface ConfirmDeleteModalProps {
    ...
    onCancel: () => void;
    /** Uses replacement language and icon for destructive overwrite flows. */
    intent?: 'delete' | 'replace';
}
...
export const ConfirmDeleteModal: React.FC<ConfirmDeleteModalProps> = ({
    ...
    const isReplacement = intent === 'replace';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                <div className="space-y-2">
                    <h3 id={titleId} className="text-title text-white">
                        {isReplacement ? 'Replace' : 'Delete'} {itemType}?
                    </h3>
                    <p className="text-body text-slate-400">
                        {isReplacement
                            ? `The selected file will replace the current ${itemType.toLowerCase()} with:`
                ...
                <div className="bg-red-500/[0.03] border border-red-500/10 p-4 rounded-lg text-left space-y-1">
                    <span className="text-label text-red-400 block">
                        {isReplacement ? '⚠️ Permanent Replacement' : '⚠️ Irreversible Action'}
                    </span>
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    >
                        {isReplacement ? 'Yes, Replace' : 'Yes, Delete'}
                    </button>
                </div>
            </div>
        </div>
